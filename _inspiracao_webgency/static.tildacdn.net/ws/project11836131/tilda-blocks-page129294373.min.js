...
function t635_updateTypingOnFuncLoad(animRecId, textHolder) {
  ...
  animatedTextElems.forEach(function (animatedText) {
    ...
    var isSBSAnimation =
      zeroElem && zeroElem.getAttribute("data-animate-sbs-event");
...
function t635_processIntersectionObserver(callback) {
  var recT809 = document.querySelector(
    '[data-record-type="809"], [data-parenttplid="809"]'
  );
  if (!recT809) {
    callback();
  } else {
...
function t635_updateAnimTextLimits(animRecId) {
  var animBlock = document.getElementById("rec" + animRecId);
  var animTextElems = animBlock
    ? animBlock.querySelectorAll(".t635__typing-text")
    : [];
  if (!animTextElems.length) return;
  animTextElems.forEach(function (elem) {
...
function t778__showMore(recid) {
  ...
  showMoreButton.addEventListener("click", function () {
    ...
    var event = document.createEvent("HTMLEvents");
    event.initEvent("displayChanged", !0, !1);
...
function t778__showSeparator(rec, cardsShowLimit) {
  ...
  Array.prototype.forEach.call(allSeparators, function (separator) {
    ...
    if (
      separator.getAttribute("data-product-separator-number") <= cardsShowLimit
    ) {
...
function t778__updateLazyLoad(recid) {
  var scrollContainer = document.querySelector(
    "#rec" + recid + " .t778__container_mobile-flex"
  );
...
function t778_initPopup(recid) {
  ...
  Array.prototype.forEach.call(allPopupLinks, function (popupLink) {
    ...
    popupLink.addEventListener("click", clickOnceHandler, { once: !0 });
    ...
    popupLink.addEventListener("click", function (event) {
      ...
      Array.prototype.forEach.call(
        popup.querySelectorAll(".js-product"),
        function (product) {
...
function t778_showPopup(recid) {
  ...
  var closeButtons = document.querySelectorAll(
    ".t-popup__close, .t778__close-text"
  );
  ...
  for (var i = 0; i < headers.length; i++) {
    headers[i].addEventListener("click", function () {
      ...
      if (element.classList.contains("t585__opened")) {
        ...
        var zoom = 1;
        if (rec && rec.classList.contains("t-record_scaled")) {
          zoom = parseFloat(getComputedStyle(rec).zoom) || 1;
        }
        setTimeout(function () {
